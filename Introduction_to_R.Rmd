...
To practice, let's use the [Maps That Changed Our World](https://www.arcgis.com/home/item.html?id=72f29810c72043358d7b2bad4c2f22c9) dataset to meet some of native data types :

```{r echo=FALSE}
options(stringsAsFactors = FALSE)
data <- read.csv2("MapsThatChangedOurWorld_StoryMap_Data.csv",sep = ";")
data
```

To import this csv file, we will use the function *read.csv2*. The columns of this csv dataset are separated by the character ";" so we use the input argument *sep* to tell the *read.csv2* to use that character as a separator string. The result is an object of type *data.frame*. *data frame* is the most popular data type in R. It stores data in a table structure, with multiple rows and columns. A *data.frame* gives the user ability to store a different data type at each column. Each row and column can have a name. In this dataset, each column has a name and the column names are stored in the first row of the csv file. 

Looking at the above table, we can see that all the columns are input as type "character". The reason why columns *Latitude* and *Longitude* are imported as characters is the presence of strings *E* and *N*. The reason why the column **Year** is imported as character is the presence of the string *AD* in the first line. We will have to process the data later on to normalize all the columns.

See also : read.csv(), read.table(), read.delim(), read.delim2().
