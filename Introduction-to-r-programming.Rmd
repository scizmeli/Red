...
## Yeni kullanıcı kabusları (Error, warning)
```{r eval=FALSE}
45 - "dffd"
```
...
objects()
```
The above command lists the objects currently stored in the memory of R (which is also called, workspace). 

To gather more information about an object, try to following utility functions : *length()*, *dim()*, *names()*, *head()*, *tail()* and *summary()*. 

To remove objects from the memory (use with care, no recovery is possible if you don't have a backup), use the function *rm()* :
```{r}
rm(a)
ls()
```
...
### Exercise
To increase the readability of the resulting `data.frame` object `f`, sort the table in descending order of `meas` values.
...
When we have a named variable, we can reference/subset its elements on using names :
```{r}
v["hotel"]
```
...
b <- 'There one containing "one word"'
a
class(b)
```
...
myclass[meas < 50] <- "low"
myclass[meas >= 50 & meas < 80] <- "medium"
myclass[meas >= 80] <- "high"

f <- data.frame(measurement=meas, class=myclass)
print(f)
```
...
### Creating a data.frame by importing CSV file
We can import csv data directly into data.frame using *read.csv()* and *read.csv2()* :
```{r}
...
### Exercise
Produce same output without using a `for` loop. 
...
To calculate sum of votes for each candidate, use :
```{r}
apply(flo[,2:11],2, mean)
```
...
| ------------- |-------------|
| cat()         | Prints contents. Used both in data import and export|
| write.csv()  | Export .csv file into a data.frame |
| jsonlite::toJSON() | Export into JSON  format|
...
| xlsx::read.xlsx() | Read and write data from/to Excel files|
| png::writePNG(), jpeg::writeJPEG(), tiff::writeTIFF()| Export RGB images|

## Database connections
The R packages *RODBC* and the more recent *DBI* offer a unified interface for various SQL databases. Individual packages *RMySQL*, *ROracle*, *RPostgreSQL* and *RSQLite* support more sophisticated operations for MySQL, Oracle, PostgreSQL and sqlite databases respectively.

It is also possible to i/o data from NoSQL databases. Refer to packages *RMongo*, *mongolite*, 
