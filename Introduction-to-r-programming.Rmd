...
Factors are used to store category information. Factors are internally stored by R as integers, but every category also have names, which are called *levels* in R.

The easiest way of creating factors is using function *factor()* :
```{r}
factor(c("asia","europe","australia","europe","america","asia","europe","asia"))
```
